    ...
    useEffect(() => {
        ...
        const updateSelection = () => {
            ...
            if (active) {
                ...
                if (window.innerWidth < 1024) setIsPropertiesOpen(true);
            } else {
            ...
            {activeTool === 'filters' ? (
                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-2 gap-2">
                    {['Grayscale', 'Sepia', 'Invert', 'Blur', 'Noise', 'Pixelate'].map(f => (
                        <button key={f} className="bg-gray-700 hover:bg-cyan-600 rounded p-2 text-[10px] flex flex-col items-center gap-1 transition-colors">
        ...
        <div className="fixed inset-0 h-[100vh] w-full flex flex-col bg-gray-950 text-white font-sans overflow-hidden overscroll-none">
            ...
            <header className="h-14 bg-gray-900/95 backdrop-blur border-b border-gray-800 flex items-center justify-between px-4 z-50 shrink-0">
                <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
                    <div className="w-8 h-8 bg-gradient-to-br from-cyan-400 to-blue-600 rounded flex items-center justify-center shrink-0">
                        <FaShapes className="text-white text-sm" />
                    </div>
                    <span className="font-bold text-sm sm:text-lg tracking-tight truncate">Lumina Studio</span>
                </div>
                <div className="flex items-center gap-2">
                    <SignedIn>
                        <button
                            ...
                            className="bg-cyan-500 hover:bg-cyan-400 text-black px-3 py-1.5 sm:px-4 rounded-full text-[10px] sm:text-xs font-bold flex items-center gap-2 transition-transform active:scale-95 shrink-0"
                        >
            ...
            <div className="flex-1 flex flex-row overflow-hidden relative">
                ...
                <div className="hidden lg:flex lg:flex-col lg:w-20 bg-gray-900 border-r border-gray-800 z-40 py-4 items-center gap-4">
                ...
                <div className="lg:hidden fixed bottom-0 left-0 w-full h-16 bg-gray-900 border-t border-gray-800 z-50 flex items-center justify-around px-2">
                    <ToolButton icon={FaMousePointer} label="Select" isActive={activeTool === 'select'} onClick={activateSelectMode} />
                    <ToolButton icon={FaPencilAlt} label="Draw" isActive={activeTool === 'draw'} onClick={activateDrawingMode} />
                    <div className="relative">
                        <ToolButton icon={FaShapes} label="Shapes" isActive={showShapesMenu} onClick={() => setShowShapesMenu(!showShapesMenu)} />
                        {showShapesMenu && (
                            <div className="absolute bottom-16 left-1/2 -translate-x-1/2 bg-gray-800 border border-gray-700 rounded-lg p-2 shadow-xl flex flex-col gap-2 w-32 z-50 mb-2">
                                <button onClick={() => handleShapeAdd('rect')} className="text-sm hover:text-cyan-400 text-left p-2 rounded hover:bg-gray-700">Rectangle</button>
                                <button onClick={() => handleShapeAdd('circle')} className="text-sm hover:text-cyan-400 text-left p-2 rounded hover:bg-gray-700">Circle</button>
                                <button onClick={() => handleShapeAdd('triangle')} className="text-sm hover:text-cyan-400 text-left p-2 rounded hover:bg-gray-700">Triangle</button>
                            </div>
                ...
                <main className="flex-1 relative bg-[#0f172a] overflow-hidden flex items-center justify-center z-10" ref={containerRef}>
                    ...
                    <div className="relative shadow-2xl transition-transform duration-300">
                        <canvas ref={canvasRef} />
                    </div>
                </main>


                <div className="hidden lg:flex w-72 xl:w-80 bg-gray-900 border-l border-gray-800 flex-col z-20">
            ...
            `} style={{ maxHeight: '50vh' }}>
                ...
                <div className="p-4 overflow-y-auto max-h-[40vh] custom-scrollbar pb-8">
                    {renderPropertiesContent()}
                </div>
            </div>
